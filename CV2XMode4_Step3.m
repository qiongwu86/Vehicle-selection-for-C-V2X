...
%     Psen_Step3 = Psen - step_dB;    % Initial sensing value
    ...
    while Na < Nc
        ...
        PSR = 0.5 * (1 + erf( (Pt - PL - Psen)./(std_dev*sqrt(2)) ) );  % Calculate PSR for current threshold. Equation (32.2)
        Spsr_n = 2 * beta * sum(PSR) ;  % Equation (32.1)
...
%     Lint_max = round(500*beta)/beta;    % Distance to the farthest interfering vehicle. Up to 1000m distances considered to speed up the calculations.
...
%     PSR = 0.5 * (1 + erf( (Pt - PL - Psen )./(std_dev*sqrt(2)) ) );   % % PSR for all distances in d_aux. Equation (11)
    ...
    for d=1:length(distance)  
        ...
        for i = 1:length(distance_int_to_rx)    
            ...
            tau = lambda;
            pSensar_I_E = 0.5 * (1 + erf(( Pt - PL_I_E - Psen )/(2^0.5*std_dev_I_E)));  % PSR for interfering and transmitting vehicles
    ...
    %p_SINR
    %int=p_SINR;
    %dlmwrite('int_100_10_23_2.txt', int, 'delimiter', ',', '-append');
return
